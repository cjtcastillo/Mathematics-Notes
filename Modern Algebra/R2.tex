...
\end{definition}
	...
	\begin{equation*}
		a_0+a_1x+a_2x^2+\cdots+a_nx^n \to (a_0,a_1,a_2,a_3,...,a_n)
	\end{equation*}
\begin{proposition}
...
\begin{ceg}
	Consider the polynomials
	\[
	f(x) = 2x^2 + 3x + 1 \quad \text{and} \quad g(x) = 4x + 1
	\]
	over $\mathbb{Z}[x]$.  
	
	- The leading coefficient of $g(x)$ is $4$, which is \emph{not} a unit in $\mathbb{Z}$.  
	- Attempting to divide the leading term:
	
	\[
	\frac{2x^2}{4x} = \frac{1}{2} x \notin \mathbb{Z}[x]
	\]
	
	- We cannot produce integer coefficients for the quotient.  
