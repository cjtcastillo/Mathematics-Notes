Consider a set $\set{1,2,...,n}$ of $n$ distinct elements. Then a permutation of $\set{1,2,...,n}$ is a bijection 
\begin{equation*}
...
\begin{remark}
	...
	\begin{equation*}
		f(1)f(2)f(3)f(4) = (12) 
	\end{equation*}
	where $f(1) = 2$, $f(2)=1$, and $f(3),f(4) = I_X$
\end{remark}
\begin{proposition}
	...
	\begin{equation*}
		|S_n| =  n!
	\end{equation*}
\end{proposition}\marginnote{Note that each permutation is treated as unique}
\begin{proof}
	By definition, a permutation is a bijection
	\begin{equation*}
		f: \set{1,2,...,n}\to \set{1,2,...,n}
	\end{equation*}
	We construct a bijection\sidenote{Note that, in this proof, we're focusing on the method in which a bijection is constructed because it allows us to directly compute the number of possible permutations i.e the number of $f \in S_n$.} by specifying the values of $f(1),f(2),...,f(n)$ sequentially. There are $n$ possible choices for $f(1)$. Once $f(1)$ is chosen, by way of injectivity, any subsequent selection must have $n-k$ possible choices for $f(k+1)$, and for the final selection of $f(n)$ there exists exactly one element to choose from. Then, by the multiplication rule, the total number of such sequences of choices is 
	\begin{equation*}
...
\end{proof}
\begin{eg}
	Take a triangle \sidenote{Permutations of geometric configurations do not guarantee symmetry after transformation. The \textit{dihedral group} $D_n$ does though.}, with 3 distinct vertices, each labeled 1,2,3 respectively. The symmetric group $S_3$ of all permutations of the triangle will have $3!$ total permutations.
	\begin{center}
